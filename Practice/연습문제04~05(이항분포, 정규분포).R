...
## 02. 패널티킥 성공확률 4/5, 10번 시도해서 7골 성공 확률? 
# 성공확률 = 4/5, 실행 횟수 = 10
n <- 10
p <- 0.8
x <- 1:n
...
## 03. 불량률 5%, 20개 조사해서 불량이 2개 이하 나올 확률?
# 불량률 5%, 실행 횟수 = 20
n <- 20
p <- 0.05
x <- 1:n
...
## 04. 치료율 20%, 20명을 치료했을때 2명 이상이 회복할 확률?
n <- 20
p <- 0.2
x <- 1:n
...
## 05. 주사위 두 개를 던졌을때, 눈금의 합이 6이 될 확률?
# 합이 6 나올 확률 5/36, 
...
### 연습문제05. 정규 분포
## 01. 전구의 수명 800일, 표준편차 40일, 전구의 수명이 750일 이하일 확률?
...
## 03. 수학성적 평균 70, 표준편차 8, 점수가 80점 이상이고 90점 이하인 학생 비율?
mu <- 70
sigma <- 8
...
## 04. 평균 1.5, 표준편차 2, H(t) = P(t <= X <= t+1)
# H(0) + H(2) ?
mu <- 1.5
sigma <- 2
